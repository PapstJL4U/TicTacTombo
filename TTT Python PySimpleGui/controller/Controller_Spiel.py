...
class Controller_Spiel(IController):
    def __init__(self, rows: int = 3, cols: int = 3, wins: int = 3):
        ...
        spieler1 = Spieler("Alice 'O' Little", 0, Farbe.Azure)
        spieler2 = Spieler("Cassandra 'X' Cain", 1, Farbe.Green)
        self._DieSpieler = [spieler1, spieler2]
        self._AnDerReihe: int = 0
        self._generiere_Stein_ID = 0
        self._Feld = Spielfeld(rows, cols, 0)
        self._Regeln = Spielregeln(wins, self._Feld)
        self._gui = gui(self)
    ...
    def neuesSpiel(self) -> None:
        ...
        self._Regeln = Spielregeln(self._wins, self._Feld)
    ...
    def Button_pressed(self, row: int = -1, col: int = -1) -> None:
        ...
        if self._Regeln.ist_gueltiger_Zug(row, col, Stein):
            ...
            self._gui.setArrow(self._AnDerReihe, self._DieSpieler[self._AnDerReihe].Farbe)

    def _gewonnen(self, sp: Spieler = None) -> None:
        sp.inc_wins()
